...
phone_number = st.text_input(
    "הכנס מספר טלפון (כולל קידומת מדינה, לדוגמה +972501234567)"
)
        ...
        if send_type == "מיידית":
            try:
                kit.sendwhatmsg_instantly(
                    phone_number, message, wait_time=10, tab_close=True
                )
            ...
            hour = st.number_input(
                "שעה לשליחה (0-23):", min_value=0, max_value=23, value=now.hour
            )
            minute = st.number_input(
                "דקה לשליחה (0-59):", min_value=0, max_value=59, value=now.minute + 1
            )
            try:
